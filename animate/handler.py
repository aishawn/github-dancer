...
def load_workflow(workflow_path):
    ...
    try:
        ...
        with open(workflow_path, 'r') as file:
            ...
            if not workflow:
                raise ValueError(f"Workflow 文件为空或无效: {workflow_path}")
            return workflow
    except FileNotFoundError as e:
        logger.error(f"❌ {e}")
        raise
    except json.JSONDecodeError as e:
        logger.error(f"❌ Workflow JSON 解析失败: {workflow_path}, 错误: {e}")
        raise Exception(f"Workflow JSON 解析失败: {e}")
    except Exception as e:
        logger.error(f"❌ 加载 Workflow 失败: {workflow_path}, 错误: {e}")
        raise
...
def handler(job):
    ...
    # 验证必需参数
    required_params = ["prompt", "seed", "width", "height", "fps", "cfg"]
    missing_params = [param for param in required_params if param not in job_input]
    if missing_params:
    ...
    if "image_path" in job_input:
        ...
        error_msg = "必须提供 image_path、image_url 或 image_base64 之一"
        logger.error(f"❌ {error_msg}")
        return {"error": error_msg}
    ...
    # 初始化 prompt 变量，避免 UnboundLocalError
    prompt = None
    ...
    try:
        ...
        error_msg = f"加载或配置 workflow 失败: {str(e)}"
        logger.error(f"❌ {error_msg}")
        return {"error": error_msg}
    
    # 验证 prompt 是否成功加载
    if prompt is None:
        error_msg = "Workflow 加载失败，prompt 为 None"
        logger.error(f"❌ {error_msg}")
        return {"error": error_msg}
